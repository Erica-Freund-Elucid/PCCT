...
# Canonical (full-vessel) paired data, length-normalized — the apples-to-apples match to
# the OQ reference, which is itself length-normalized on the full traced vessel.
...
def main():
    ...
    for var, (label, oq_bias, oq_ci, oq_loa) in OQ.items():
        ...
        ax.set_title(f"{label} — canonical, length-normalized vs OQ\n{verdict}",
                     fontsize=11)
